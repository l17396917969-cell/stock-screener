...
from core.deepseek_analyzer import generate_short_term_analysis, generate_watchlist_diagnosis
...
def execute_step1_macro(user_id, api_key=None, ds_key=None):
    try:
        ...
        _emit_log(user_id, f"[SYS] 正在调用 {backend} 大模型进行宏观定调 + 成分股分析...", "info")
        ...
        if api_key: SCRENNER_CONFIG["GEMINI_API_KEY"] = api_key
        if ds_key: SCRENNER_CONFIG["DS_API_KEY"] = ds_key
        ...
        _emit_log(user_id, f"[PASS] AI宏观定调完毕，推荐板块: {', '.join(hot_sectors)}"  
                  f"，共覆盖 {total_stocks} 只候选股股票。", "pass")
...
def execute_step2_fetch(user_id, selected_sectors):
    try:
        state_mgr.update_state(user_id, {"is_analyzing": True, "error": None, "selected_sectors": selected_sectors})
        ...
        if ai_result and ai_result.get("sectors_detail"):
            _emit_log(user_id, f"📋 正在从 AI 分析结果中读取 [{', '.join(selected_sectors)}] 的成分股...", "info")
            extracted_stocks, extracted_infos = get_stocks_from_sectors(selected_sectors, ai_result)
            all_stocks_set.update(extracted_stocks)
            stock_infos.update(extracted_infos)
        else:
            _emit_log(user_id, f"📋 正在从底层数据源实时获取 [{', '.join(selected_sectors)}] 的成分股...", "info")
        ...
        _emit_log(user_id, f"[PASS] 成分股提取完成，共 {len(all_stocks)} 只候选股，可开始深度量化审计。", "pass")
...
def analyze_single_stock_sync(user_id, code, index_hist=None, force=False):
    ...
    if not passed:
        ...
        _emit_log(user_id, f"[PASS] {name} ({code}) 通过深筛，正在汇总量化数据...", "pass")
        if score_report:
             ...
             cost = shares * price
             score_report['trade_plan'] = f"建议买入 {shares} 股 ({shares//100}手)，约 {price:.2f}元，占 {cost:.2f}元。"
             
        result = {"passed": True, "reason": "通过全部关卡", "score_report": score_report}
...
def execute_step3_batch(user_id):
    try:
        ...
        state_mgr.update_state(user_id, {"batch_progress": {"total": len(candidates), "current": len(candidates) - len(un_analyzed)}})
        _emit_log(user_id, f"[SYS] 启动全量后台审计。共需排雷 {len(un_analyzed)} 只标的...", "warn")
        index_hist = get_index_data()

        for i, code in enumerate(un_analyzed, 1):
             if not state_mgr.get_field(user_id, "is_analyzing"): break 
             analyze_single_stock_sync(user_id, code, index_hist=index_hist)
             with state_mgr._get_user_entry(user_id)["lock"]:
                 state_mgr._get_user_entry(user_id)["state"]["batch_progress"]["current"] += 1
             time.sleep(0.5)
...
def step1():
    ...
    if not ds_key or not api_key:
        db = get_db()
        secrets = db.execute("SELECT ds_key_enc, gemini_key_enc FROM user_secrets WHERE user_id = ?", (current_user.id,)).fetchone()
        if secrets:
            if not ds_key and secrets['ds_key_enc']: ds_key = decrypt_key(secrets['ds_key_enc'])
            if not api_key and secrets['gemini_key_enc']: api_key = decrypt_key(secrets['gemini_key_enc'])
    state_mgr.set_field(current_user.id, "logs", [])
    threading.Thread(target=execute_step1_macro, args=(current_user.id, api_key, ds_key)).start()
    return jsonify({"success": True})
...
def step2():
    ...
    if not selected: return jsonify({"success": False, "message": "请至少选择一个板块"})
    threading.Thread(target=execute_step2_fetch, args=(current_user.id, selected)).start()
    return jsonify({"success": True})
...
def deepseek_analysis():
    ...
    if not api_key:
        db = get_db()
        secrets = db.execute("SELECT ds_key_enc FROM user_secrets WHERE user_id = ?", (current_user.id,)).fetchone()
        if secrets and secrets['ds_key_enc']: api_key = decrypt_key(secrets['ds_key_enc'])
        if not api_key: api_key = os.environ.get('DS_API_KEY') or SCRENNER_CONFIG.get("DS_API_KEY")
    if not api_key: return jsonify({"success": False, "message": "未配置 DeepSeek API Key"})
    if not code: return jsonify({"success": False, "message": "缺少股票代码"})
    res = state_mgr.get_field(current_user.id, "analysis_results", {}).get(code)
    if not res: res = analyze_single_stock_sync(current_user.id, code)
    index_hist = get_index_data()
    from core.data_fetcher import get_stock_data_yf
    yf_data = get_stock_data_yf(code, index_hist=index_hist)
    if not yf_data: return jsonify({"success": False, "message": "无法获取最新行情数据进行研判"})
    try:
...
def get_watchlist():
    ...
    if not watchlist: return jsonify({"success": True, "watchlist": {}})
    try:
        import akshare as ak
        import pandas as pd
        
        # 区分A股和美股
        us_codes = {code: info for code, info in watchlist.items() if not code.isdigit()}
        cn_codes = {code: info for code, info in watchlist.items() if code.isdigit()}
        
        # 获取A股实时数据 (需要禁用代理)
        if cn_codes:
            old_http, old_https, old_all = os.environ.get('HTTP_PROXY'), os.environ.get('HTTPS_PROXY'), os.environ.get('ALL_PROXY')
            ...
            for code, info in cn_codes.items():
                ...
                if not row.empty:
                    info['current_price'] = float(row.iloc[0]['最新价']) if pd.notnull(row.iloc[0]['最新价']) else None
                    info['pct_change'] = float(row.iloc[0]['涨跌幅']) if pd.notnull(row.iloc[0]['涨跌幅']) else None
                else:
                ...
                try:
                    ...
                    if live.get('last_price'):
                        ...
                        if prev_close and prev_close > 0:
                            info['pct_change'] = ((info['current_price'] - prev_close) / prev_close) * 100
                    else:
                        info['current_price'] = info['pct_change'] = None
                except Exception as e:
...
def add_to_watchlist():
    ...
    if not code: return jsonify({"success": False, "message": "缺失股票代码"})
    entry_price = None
    if name == code or not sectors:
        ...
        try:
            ...
            if not hist.empty: entry_price = float(hist['Close'].iloc[-1])
            if not sectors: sectors = [ticker.info.get('sector', '未知板块')]
        except:
            if not sectors: sectors = ["未知板块"]
    watchlist_repo.add_stock(current_user.id, code, {"name": name, "sectors": sectors, "added_at": time.time(), "entry_price": entry_price})
...
def audit_watchlist_stock():
    data = request.json or {}
    code = data.get('code')
    if not code: return jsonify({"success": False, "message": "无效的股票代码"})
    result = analyze_single_stock_sync(current_user.id, code, force=True)
    if result and result.get('score_report'): watchlist_repo.save_audit_report(current_user.id, code, result.get('score_report'))
    return jsonify({"success": True, "result": result})
...
def update_watchlist_position():
    ...
    if not code: return jsonify({"success": False, "message": "无效的股票代码"})
    return jsonify({"success": watchlist_repo.update_position(current_user.id, code, status, cost_price, shares)})
...
def ai_diagnose_watchlist_stock():
    ...
    if not ds_key:
        db = get_db()
        secrets = db.execute("SELECT ds_key_enc FROM user_secrets WHERE user_id = ?", (current_user.id,)).fetchone()
        if secrets and secrets['ds_key_enc']: ds_key = decrypt_key(secrets['ds_key_enc'])
        if not ds_key: ds_key = os.environ.get('DS_API_KEY') or SCRENNER_CONFIG.get("DS_API_KEY")
    if not code or not ds_key: return jsonify({"success": False, "message": "缺失参数"})
    all_wl = watchlist_repo.get_all(current_user.id)
    if code not in all_wl: return jsonify({"success": False, "message": "未找到自选股"})
    from core.data_fetcher import get_stock_data_yf
    yf_data = get_stock_data_yf(code, index_hist=get_index_data())
    if not yf_data: return jsonify({"success": False, "message": "数据获取失败"})
    try:
...
def get_watched_sectors():
    return jsonify({"success": True, "sectors": sector_watchlist_repo.get_all(current_user.id)})

@app.route('/api/watched_sectors/add', methods=['POST'])
@login_required
def add_watched_sector():
    name = (request.json or {}).get('sector_name')
    if not name: return jsonify({"success": False, "message": "缺失板块名称"})
...
def remove_watched_sector():
    name = (request.json or {}).get('sector_name')
    if not name: return jsonify({"success": False, "message": "缺失板块名称"})
    return jsonify({"success": sector_watchlist_repo.remove_sector(current_user.id, name)})
...
def settings():
    db = get_db()
    secrets = db.execute("SELECT ds_key_enc, gemini_key_enc FROM user_secrets WHERE user_id = ?", (current_user.id,)).fetchone()
    return render_template('settings.html', has_ds_key=bool(secrets and secrets['ds_key_enc']), has_gemini_key=bool(secrets and secrets['gemini_key_enc']))

@app.route('/settings/keys', methods=['POST'])
@login_required
def update_keys():
    ds_key, gemini_key = request.form.get('ds_key', '').strip(), request.form.get('gemini_key', '').strip()
    db = get_db()
    ds_enc, gem_enc = encrypt_key(ds_key) if ds_key else None, encrypt_key(gemini_key) if gemini_key else None
    existing = db.execute("SELECT * FROM user_secrets WHERE user_id = ?", (current_user.id,)).fetchone()
    if existing:
        if ds_enc: db.execute("UPDATE user_secrets SET ds_key_enc = ?, updated_at = ? WHERE user_id = ?", (ds_enc, time.time(), current_user.id))
        if gem_enc: db.execute("UPDATE user_secrets SET gemini_key_enc = ?, updated_at = ? WHERE user_id = ?", (gem_enc, time.time(), current_user.id))
    else:
        db.execute("INSERT INTO user_secrets (user_id, ds_key_enc, gemini_key_enc, updated_at) VALUES (?, ?, ?, ?)", (current_user.id, ds_enc, gem_enc, time.time()))
    db.commit()
...
@login_required
def update_password():
    curr_pass, new_pass, conf_pass = request.form.get('current_password'), request.form.get('new_password'), request.form.get('confirm_password')
    if new_pass != conf_pass: flash('两次输入的新密码不一致', 'error'); return redirect(url_for('settings'))
    db = get_db()
    user_row = db.execute("SELECT password_hash FROM users WHERE id = ?", (current_user.id,)).fetchone()
    if not check_password_hash(user_row['password_hash'], curr_pass): flash('当前密码错误', 'error'); return redirect(url_for('settings'))
    db.execute("UPDATE users SET password_hash = ?, must_change_password = 0 WHERE id = ?", (generate_password_hash(new_pass), current_user.id))
    db.commit()
...
@admin_required
def admin_users():
    return render_template('admin_users.html', users=get_db().execute("SELECT * FROM users ORDER BY created_at DESC").fetchall())

@app.route('/admin/users/add', methods=['POST'])
@login_required
@admin_required
def admin_add_user():
    user, pwd, is_admin = request.form.get('username'), request.form.get('password'), 1 if request.form.get('is_admin') else 0
    if not user or not pwd: flash('必填项缺失', 'error'); return redirect(url_for('admin_users'))
    try:
        db = get_db()
        db.execute("INSERT INTO users (username, password_hash, is_active, is_admin, must_change_password, created_at) VALUES (?, ?, ?, ?, ?, ?)", (user, generate_password_hash(pwd), 1, is_admin, 1, time.time()))
        db.commit()
        flash(f'用户 {user} 已添加', 'success')
    except Exception as e: flash(f'失败: {e}', 'error')
...
def admin_toggle_user(user_id):
    if user_id == current_user.id: flash('不能操作自己', 'error')
    else:
        db = get_db()
...
def admin_reset_password(user_id):
    ...
    if not pwd: flash('密码不能为空', 'error')
    else:
        db = get_db()
        db.execute("UPDATE users SET password_hash = ?, must_change_password = 1 WHERE id = ?", (generate_password_hash(pwd), user_id))
        db.commit()
        flash('密码已重置', 'success')
    return redirect(url_for('admin_users'))
